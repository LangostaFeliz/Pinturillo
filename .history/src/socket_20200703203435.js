    ...
    io.on('connection',socket => {
        ...
        socket.on('disconnect',function(data){
          ...
          if(sistema.userList.length==0){
            console.log("reinica el servidor");
            sistema.clearData();
          }
        })
        ...
        socket.on('start timeTurn',function(data){
           ...
           console.log(data);
             ...
             timerTurn= setInterval(function (){
               ...
               socket.emit('check timeTurn',count);
               },100)
        ...
        socket.on('choose word',()=>{
          ...
          socket.broadcast.emit('show textBox');
          socket.emit('hide textBox');
        })
        ...
        socket.on('next turn or round',()=>{
          sistema.clearLine_history();
          console.log("ROUNDA:"+sistema.round+"  TURN:"+sistema.turn)

          io.sockets.emit('update user',sistema.userList);
          
          if(socket.id==sistema.userList[sistema.turn].id)
          {console.log("turno anteior:"+sistema.turn);
          sistema.turn++;
          console.log("Turno actual:"+sistema.turn);
          //console.log('turn is '+sistema.userList[sistema.turn]);
          if(sistema.round>=3){
          ...
          if(sistema.turn>=sistema.userList.length){
           // io.sockets.emit('next round',sistema.round);
            sistema.turn=0;
            sistema.round++;
            socket.to(sistema.userList[sistema.turn].id).emit('new turn',sistema.userList[sistema.turn]);
          } else{

            socket.to(sistema.userList[sistema.turn].id).emit('new turn');
          }
          console.log("Ya te toca a ti:"+socket.id);
          }else{
            console.log("Aun no te toca:"+socket.id);
          }
        })

        socket.on('time show score',()=>{
          var count=10;
          timerTurn=setInterval((()=>{
            count--;
            io.sockets.emit('time show score',count);
          }),1000)
        })

        socket.on('show score',()=>{
          io.sockets.emit('show score',sistema.userList);
        })

        socket.on('delete canva',()=>{
          sistema.clearLine_history();
        })
